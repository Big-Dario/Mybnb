...
function submitListing() {
  ...
  const loc  = document.getElementById('hLocation').value;
  const price = document.getElementById('hPrice').value;
  const host = document.getElementById('hHostName').value;
  if (!name || !loc || !price || !host) { showToast('Please fill in required fields (*)','error'); return; }
  const imgs = [
  ...
  const newProp = {
    ...
    title: document.getElementById('hName').value,
    ...
    desc: document.getElementById('hDesc').value || 'A wonderful property available for booking.'
  };
...
function getAmenityTags() {
  const tags = [];
  if (document.getElementById('aWifi').checked) tags.push('WiFi');
  if (document.getElementById('aPool').checked) tags.push('Pool');
  if (document.getElementById('aParking').checked) tags.push('Parking');
  return tags.length ? tags : ['WiFi'];
}
